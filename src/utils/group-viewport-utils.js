...
export const getGroupViewportState = (groupViewportHeight, groups) => {
  let { groupVisibleStartIdx, groupVisibleEndIdx } = getGroupVisibleBoundaries(groupViewportHeight, 0, groups);
  return {
...
export const getGroupVisibleBoundaries = (groupViewportHeight, scrollTop, groups) => {
  let groupIdx = 0;
  let groupsHeight = 0;
...
export const getGroupHeight = (group) => {
  const { isExpanded, rows } = group;
  const rowsHeight = isExpanded && Array.isArray(rows) ? rows.length * ROW_HEIGHT : 0;
  return GROUP_HEADER_HEIGHT + rowsHeight;
};

export const getGroupsHeight = (groups, startIndex, endIndex) => {
  let groupsHeight = 0;
  for (let i = startIndex; i < endIndex; i++) {
    const group = groups[i];
    groupsHeight += getGroupHeight(group);
  }
  return groupsHeight;
};